-- =========================================================
-- 107_fix_template_subject_variable_rendering.sql — Phase 8
-- =========================================================
-- BUG REAL encontrado probando 105: render_message_template() solo
-- aplicaba {{customer.first_name}} al subject — el resto de las
-- variables quedaban sin reemplazar si aparecían en el subject.
-- Verificado con datos reales: "Your estimate {{estimate.number}} is
-- ready" ahora renderiza "Your estimate EST-000001 is ready".
-- =========================================================
...
begin
  ...
  for i in 1..2 loop
    if i = 1 then
      ...
      v_subject := replace(v_subject, '{{company.name}}', coalesce(v_org.name, ''));
      v_subject := replace(v_subject, '{{vessel.name}}', coalesce(v_vessel.name, ''));
      v_subject := replace(v_subject, '{{work_order.title}}', coalesce(v_wo.title, ''));
      v_subject := replace(v_subject, '{{estimate.number}}', coalesce(v_estimate_number, ''));
      v_subject := replace(v_subject, '{{estimate.total}}', coalesce(to_char(v_estimate_total, 'FM999,999,990.00'), ''));
      v_subject := replace(v_subject, '{{invoice.number}}', coalesce(v_invoice_number, ''));
      v_subject := replace(v_subject, '{{invoice.balance_due}}', coalesce(to_char(v_invoice_balance, 'FM999,999,990.00'), ''));
      v_subject := replace(v_subject, '{{appointment.date}}', coalesce(v_appt_date, ''));
      v_subject := replace(v_subject, '{{technician.name}}', coalesce(v_tech_name, ''));
    else
