-- =========================================================
-- 173_remove_test_fault_injection_from_convert_website_lead.sql — Phase 11 closure
-- =========================================================
-- Quita las 2 ramas de falla forzada (FORCE_FAIL_AFTER_CUSTOMER/
-- FORCE_FAIL_AFTER_VESSEL) usadas temporalmente para verificar los
-- escenarios D/E de la sección 23 con datos reales — deja la versión
-- de producción limpia.
--
-- Verificado con datos reales (con las ramas de prueba activas,
-- luego quitadas): falla forzada después de crear el customer -> el
-- lead queda 'failed' con el error real persistido de verdad (fix de
-- 172), sin customer huérfano; falla forzada después de crear el
-- vessel -> lead 'failed', sin vessel huérfano, el customer se
-- revierte atómicamente también; reintento limpio tras cada una
-- completa la conversión con exactamente un customer/vessel/service_
-- request nuevo, sin duplicados, conversion_error se limpia al
-- convertir con éxito.
-- =========================================================
