...
export async function GET(request:NextRequest){
 ...
 if(process.env.LUZ_FOLLOWUPS_ENABLED!=='true'||!process.env.RESEND_API_KEY||!process.env.RESEND_INBOUND_DOMAIN)return NextResponse.json({error:'Follow-up delivery is not enabled'},{status:503});
 ...
 for(let i=0;i<10;i++){
  ...
  const replyTo=replyAddress(job.conversationId,process.env.RESEND_INBOUND_DOMAIN);
  if(!replyTo){await db.from('luz_estimate_followups').update({status:'failed',reason:'Reply routing is not configured'}).eq('id',job.jobId);failed++;continue;}
  const result=await getEmailProvider().sendEmail({to:job.recipient,from:job.fromName?`${job.fromName} <${job.fromEmail}>`:job.fromEmail,replyTo,subject:job.subject,text:job.body,html:renderOperationalEmail({body:job.body,signature:job.signature,portalUrl:`${getSiteUrl()}/estimates/${job.estimateId}`}),idempotencyKey:job.messageId});
