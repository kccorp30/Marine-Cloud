...
function ResetPasswordContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Supabase already accepted and saved the password. The profile marker is
    // secondary state and must not turn a successful password change into a
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {isWelcome && (
            <div className="pt-2 text-center">
              <p className="mb-2 text-[11px] text-cool-gray">Already have a Marine Cloud account?</p>
              <button
                type="button"
                onClick={goToSignIn}
                className="auth-text-link inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 hover:border-gold/30 hover:bg-white/[.03]"
              >
                Sign in with email and password <span aria-hidden="true">→</span>
              </button>
            </div>
          )}
        </form>
