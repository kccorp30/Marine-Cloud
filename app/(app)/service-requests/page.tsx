...
export default async function ServiceRequestsPage() {
  ...
  const mediaByRequest = new Map<string, Awaited<ReturnType<typeof listEntityMedia>>>();
  await Promise.all((requests ?? []).map(async (request) => {
    mediaByRequest.set(request.id, await listEntityMedia('service_request', request.id));
  }));
        ...
        {(requests ?? []).map((r) => {
          ...
          return (
            <Card key={r.id}>
              ...
              <div className="mt-4"><EntityMediaGallery items={mediaByRequest.get(r.id) ?? []} title="Customer photos" /></div>
