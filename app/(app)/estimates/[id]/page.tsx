...
import { addLineItem, removeLineItem, updateVersionDetails, reviseEstimateAction, cancelEstimateAction, convertToWorkOrderAction } from '@/lib/estimates/actions';
...
export default async function EstimateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      {current && isCustomer && <><EstimateDocument estimate={estimate} version={current} locale={locale}/><PaymentMethods estimateId={estimate.id} locale={locale}/></>}
      {current && !isCustomer && (
        <Card>
        ...
        <Card>
          ...
          <ActionForm action={updateVersionDetails} className="mt-6 pt-4 border-t border-white/10 space-y-3">
            ...
            <Field label="Customer Message">
              <EstimateMessageEditor defaultValue={current.customerMessage ?? ''} />
            </Field>
      ...
      {isStaff && estimate.status === 'approved' && !estimate.workOrderId && (
        <Card>
          <form action={convertToWorkOrderAction.bind(null, estimate.id)}>
