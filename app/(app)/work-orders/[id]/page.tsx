...
import { DispatchMissionCard } from '@/components/work-orders/DispatchMissionCard';
import { MissionRail } from '@/components/work-orders/MissionRail';
...
export default async function WorkOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="mb-7"><MissionRail status={wo.current_status} /></div>
      ...
      {isStaff && (
        <div className="mb-8">
          <DispatchMissionCard
            ...
            assignment={(assignments ?? [])[0] as any}
            appointment={(appointments ?? []).length ? (appointments ?? [])[appointments!.length - 1] as any : null}
            locale={locale}
          />
        </div>
      )}
