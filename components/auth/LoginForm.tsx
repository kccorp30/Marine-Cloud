...
export function LoginForm({ t }: { t: Dictionary }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // A successful password authentication is authoritative proof that this
    // account has a working password. Persist that fact for future invite UX.
    const credentialState = await fetch('/api/auth/credential-state', { method: 'POST' });
    if (!credentialState.ok) {
      await supabase.auth.signOut();
      setLoading(false);
      setError('Your credentials were accepted, but account setup could not be finalized. Please try again.');
      return;
    }
